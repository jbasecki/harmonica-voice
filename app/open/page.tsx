...
function OpenContent() {
  ...
  const message = searchParams.get('message') || ''; // THE HERO
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#000', position: 'relative', overflowX: 'hidden', fontFamily: 'serif' }}>
      ...
      {/* THE VOICE: Ensure reveal_chime.mp3 is in your bucket */}
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', paddingTop: '10vh' }}>
        <button onClick={() => setIsMuted(!isMuted)} style={{ position: 'absolute', top: '20px', right: '20px', border: '1px solid gold', background: 'none', color: 'gold', borderRadius: '50%', width: '40px', height: '40px', cursor: 'pointer', zIndex: 10 }}>
          {isMuted ? '🔇' : '🔊'}
        </button>
        
        {/* YOUR MESSAGE: THE CENTERPIECE */}
