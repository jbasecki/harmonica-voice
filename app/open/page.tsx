...
function OpenContent() {
  ...
  const message = searchParams.get('message') || ''; // CATCH THE HEART
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#000', position: 'relative', overflowX: 'hidden', fontFamily: 'serif' }}>
      {/* RESTORED CONTINUITY: The chosen video atmosphere */}
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', paddingTop: '10vh' }}>
        <button onClick={() => setIsMuted(!isMuted)} style={{ position: 'absolute', top: '20px', right: '20px', border: '1px solid gold', background: 'none', color: 'gold', borderRadius: '50%', width: '40px', height: '40px', cursor: 'pointer' }}>
          {isMuted ? '🔇' : '🔊'}
        </button>
        
        {/* THE MAIN EVENT: THE USER'S ORIGINAL WORDS */}
        <div style={{ maxWidth: '750px', textAlign: 'center', marginBottom: '8vh', padding: '40px', background: 'rgba(0,0,0,0.6)', borderRadius: '30px', border: '1px solid rgba(212,175,55,0.3)' }}>
          <p style={{ color: '#D4AF37', fontSize: '1.8rem', fontStyle: 'italic', lineHeight: '1.6', letterSpacing: '1px' }}>
            "{decodeURIComponent(message)}"
          </p>
        </div>

        <h2 style={{ color: '#D4AF37', letterSpacing: '15px', fontSize: '0.7rem', opacity: 0.6, marginBottom: '40px' }}>HARMONICA</h2>
        
        <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', flexWrap: 'wrap', maxWidth: '1100px' }}>
          {wordsArray.map((w, i) => (
            <div key={i} className="group" onMouseEnter={playSound} style={{ textAlign: 'center', cursor: 'pointer' }}>
              <div style={{ display: 'flex', border: '1px solid #D4AF37', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 0 40px rgba(212, 175, 55, 0.3)', background: 'rgba(0,0,0,0.8)' }}>
                {/* LARGE ICONS: Rule First/One-Before-Last with Uppercase 5 suffix */}
                <img src={`${bucketUrl}/${w.first}5.png`} style={{ width: '100px', height: '140px', objectFit: 'cover' }} />
                <img src={`${bucketUrl}/${w.oneBeforeLast}5.png`} style={{ width: '100px', height: '140px', objectFit: 'cover' }} />
              </div>
              {/* SECRET REVEAL: Hidden until interaction */}
