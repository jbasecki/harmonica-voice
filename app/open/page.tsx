...
function OpenContent() {
  ...
  const vibe = searchParams.get('vibe') || '14'; // RESTORES CONTINUITY
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#000', position: 'relative', overflowX: 'hidden' }}>
      {/* CONTINUITY: Video background stays fixed */}
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', paddingTop: '10vh' }}>
        ...
        <h1 style={{ color: '#D4AF37', letterSpacing: '20px', fontSize: '3rem', marginBottom: '8vh' }}>HARMONICA</h1>
          ...
          {wordsArray.map((w, i) => (
            <div key={i} className="group" onMouseEnter={playSound} style={{ textAlign: 'center', cursor: 'pointer' }}>
              <div style={{ display: 'flex', border: '2px solid #D4AF37', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 0 40px rgba(212, 175, 55, 0.4)', background: 'rgba(0,0,0,0.8)' }}>
                <img src={`${bucketUrl}/${w.first}5.png`} style={{ width: '130px', height: '185px', objectFit: 'cover' }} />
                <img src={`${bucketUrl}/${w.oneBeforeLast}5.png`} style={{ width: '130px', height: '185px', objectFit: 'cover' }} />
              </div>
              <p className="secret-label" style={{ color: '#D4AF37', marginTop: '15px', letterSpacing: '4px', fontSize: '0.9rem', opacity: 0, transition: '0.5s', transform: 'translateY(10px)' }}>
                {w.original}
              </p>
            </div>
          ))}
