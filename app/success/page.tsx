...
export default function SuccessPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#D4AF37', fontFamily: 'serif', textAlign: 'center', padding: '20px' }}>
      ...
      {/* SUCCESS BACKGROUND: MUSHROOM SANCTUARY VIDEO */}
      ...
      <div style={{ position: 'relative', zIndex: 2, background: 'rgba(0,0,0,0.8)', padding: '60px', borderRadius: '40px', border: '2px solid #D4AF37', maxWidth: '650px', backdropFilter: 'blur(15px)', boxShadow: '0 0 50px rgba(212,175,55,0.2)' }}>
        <h1 style={{ letterSpacing: '10px', fontSize: '2.8rem', marginBottom: '20px', fontWeight: 'normal' }}>GIFT SEALED</h1>
        ...
        <div style={{ background: 'rgba(212,175,55,0.05)', padding: '30px', borderRadius: '20px', border: '1px dashed rgba(212,175,55,0.5)', marginBottom: '40px' }}>
          ...
          <p style={{ fontWeight: 'bold', fontSize: '1rem', wordBreak: 'break-all', color: '#fff' }}>https://harmonica-voice.vercel.app/open?id=demo</p>
        </div>

        <button 
          onClick={() => window.location.href = '/composer'} 
          style={{ background: '#D4AF37', color: '#000', border: 'none', padding: '15px 50px', borderRadius: '50px', cursor: 'pointer', letterSpacing: '3px', fontWeight: 'bold', fontSize: '0.9rem', transition: '0.3s' }}>
          RETURN TO STUDIO
        </button>
      </div>
    </main>
