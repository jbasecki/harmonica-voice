...
function ComposerContent() {
  ...
  // Carry the vibe choice from the landing page
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#000', color: '#D4AF37', position: 'relative', overflowX: 'hidden' }}>
      {/* RESTORED: Dynamic Background MP4 */}
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
        <div style={{ marginBottom: '20px', display: 'flex', gap: '15px', alignItems: 'center' }}>
          <button onClick={() => setZoom(Math.max(0.7, zoom - 0.1))} style={{ background: 'none', border: '1px solid gold', color: 'gold', borderRadius: '50%', width: '35px', height: '35px' }}>-</button>
          <span style={{ fontSize: '0.8rem', letterSpacing: '3px' }}>SCALE ART</span>
          <button onClick={() => setZoom(Math.min(1.5, zoom + 0.1))} style={{ background: 'none', border: '1px solid gold', color: 'gold', borderRadius: '50%', width: '35px', height: '35px' }}>+</button>
        </div>

        {/* STASH PREVIEW: Using First/One-Before-Last + '5' suffix */}
        ...
        <div style={{ width: '100%', maxWidth: '750px', padding: '30px', background: 'rgba(15, 15, 15, 0.9)', borderRadius: '25px', border: '1px solid #333' }}>
          ...
          <div style={{ borderTop: '1px solid #222', paddingTop: '20px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {words.map((word, i) => (
              <span key={i} onClick={() => toggleWord(word)} style={{ cursor: 'pointer', padding: '6px 12px', borderRadius: '6px', background: selectedWords.includes(word) ? '#D4AF37' : 'transparent', color: selectedWords.includes(word) ? '#000' : '#D4AF37', border: '1px solid #D4AF37' }}>{word}</span>
            ))}
          </div>
        </div>
        <button onClick={() => window.location.href = `/orb?vibe=${vibe}&tiles=${encodeURIComponent(selectedWords.join(','))}`} style={{ marginTop: '40px', padding: '15px 70px', background: 'transparent', border: '1px solid #D4AF37', color: '#D4AF37', borderRadius: '50px', cursor: 'pointer', letterSpacing: '8px' }}>CREATE HARMONICA</button>
      </div>
    </main>
  );
}
