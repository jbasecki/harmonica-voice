    ...
    <main style={{ minHeight: '100vh', background: '#000', color: '#D4AF37', position: 'relative', overflowX: 'hidden', fontFamily: 'serif' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px' }}>
        ...
        <div style={{ marginBottom: '50px', textAlign: 'center' }}>
          <p style={{ fontSize: '0.7rem', letterSpacing: '4px', marginBottom: '20px', opacity: 0.8 }}>CHOICE A: PREVIEW ESTABLISHED ATMOSPHERE</p>
        ...
        <div style={{ textAlign: 'center', marginBottom: '60px' }}>
           ...
           <button onClick={() => setIsAiLoading(true)} style={{ padding: '18px 40px', border: '1px solid #D4AF37', background: 'rgba(212,175,55,0.1)', color: '#D4AF37', borderRadius: '50px', cursor: 'pointer', fontWeight: 'bold' }}>
             {isAiLoading ? '⌛ ANALYZING MESSAGE...' : '✨ GENERATE CUSTOM AI VIDEO'}
           </button>
        </div>

        <button onClick={() => window.location.href = `/open?${searchParams.toString()}&vibe=${vibe}`} style={{ padding: '22px 100px', background: '#D4AF37', color: '#000', borderRadius: '50px', fontWeight: 'bold', letterSpacing: '8px', cursor: 'pointer' }}>
          SEAL & SHARE
        </button>
      </div>
    </main>
